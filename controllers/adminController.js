...
const socketServer = new WebSocketServer({port: 8888})
...
socketServer.on('connection', ws => {
    ...
    async function getSales() {
        try{
            ...
            const data = await client.db('db_staycation').collection('items').find({}).toArray()
...
module.exports = {
    ...
    deleteCategory: async (req, res) => {
        try {
            const {id} = req.params
            const category = await Category.findOne({_id: id}) // _id adalah nama kolom di mongodb
    ...
    addBank: async (req, res) => {
        try {
            ...
            await Bank.create({
                    name, nameBank, nomorRekening, imageUrl: `images/${
                    req.file.filename
                }`
            });
            req.flash('alertMessage', 'Success Add Category');
    ...
    deleteBank: async (req, res) => {
        try {
            const {id} = req.params
            const bank = await Bank.findOne({_id: id}) // _id adalah nama kolom di mongodb
            await fs.unlink(path.join(`public/${bank.imageUrl}`));
            await bank.remove()
    ...
    addFeature: async (req, res) => {
        try {
            ...
            if(!req.file){
                req.flash('alertMessage', 'Image Empty');
                req.flash('alertStatus', 'danger');
                res.redirect(`/admin/item/show-detail-item/${itemId}`)
            }
            const feature = await Feature.create({
                    name, qty, itemId, imageUrl: `images/${
                    req.file.filename
                }`
            });
    ...
    deleteFeature: async (req, res) => {
        ...
        try {
            const feature = await Feature.findOne({_id: id}) // _id adalah nama kolom di mongodb
    ...
    addActivity: async (req, res) => {
        try {
            ...
            const activity = await Activity.create({
                    name, type, itemId, imageUrl: `images/${
                    req.file.filename
                }`
            });
    ...
    deleteActivity: async (req, res) => {
        const {id, itemId} = req.params
        try {
            const activity = await Activity.findOne({_id: id}) // _id adalah nama kolom di mongodb
